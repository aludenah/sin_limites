...
const root=path.join(__dirname,'..'),files=['courses.js','history-catalog.js','peru-catalog.js','lenguaje-catalog.js','history-progress.js','app.js'];
...
async function test(){
 ...
 for(let n=1;n<=6;n++){run('setTopic('+(n-1)+')');assert.equal(h.redirects.at(-1),`historia-del-peru-capitulo-0${n}.html?v=20260918-language1`);}
 ...
 assert.equal(c.cloud.get('users/student/progress/navigation').catalogVersion,5);
 ...
 assert.match(home.run('renderProgressPanel()'),/Historia del Perú · Capítulo 3/);home.run('openProgressChapter()');assert.equal(home.redirects.at(-1),'historia-del-peru-capitulo-03.html?v=20260918-language1');
 ...
 for(let n=1;n<=6;n++){
  ...
  assert.deepEqual(direct.redirects,[`index.html?chapter=historia-del-peru-capitulo-0${n}&v=20260918-language1`]);
  const entry=harness(files,{search:`?chapter=historia-del-peru-capitulo-0${n}`});await entry.signIn({uid:'new'});assert.equal(entry.run('state.view'),'mode');entry.run("selectStudyMode('progressive')");assert.equal(entry.redirects.at(-1),`historia-del-peru-capitulo-0${n}.html?v=20260918-language1`);
 ...
 const ctx={window:{}};for(const f of ['peru-catalog.js','history-catalog.js','lenguaje-catalog.js'])vm.runInNewContext(fs.readFileSync(path.join(root,f),'utf8'),ctx);
 ...
 assert.equal(Object.keys(ctx.window.tracked).length,20);for(let n=1;n<=6;n++)assert.equal(ctx.window.tracked[`historia-del-peru-capitulo-0${n}`].items,10);
