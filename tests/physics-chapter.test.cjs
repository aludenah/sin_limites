...
async function test(){
  ...
  assert.equal(JSON.parse(h.local.get('sin-limites:student:fisica-capitulo-01')).pending,true);
  ...
  const remote=h.cloud.get('student/fisica-capitulo-01');
  ...
  assert.equal(h.cloud.get('student/fisica-capitulo-01').chapterCompleted,true);
  assert.equal(JSON.parse(h.local.get('sin-limites:student:fisica-capitulo-01')).pending,false);
