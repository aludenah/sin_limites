...
function dimensionOneFormulas(card,panels){
 ...
 if(card==='friction'){
  ...
  assert.ok(steps[1].includes('M^0L^0T^0=1'),'The second step includes the substitution and its simplified result');
  assert.match(panels[1],/<dt>Dimensión de μₖ<\/dt><dd>\\\(1\\\)<\/dd>/,'One Next action reaches the coefficient dimension');
...
async function chapterIntegration(){
 ...
 assert.equal(h.run('LESSONS.length'),4);assert.equal(h.run('CONTENT.workedExamples.length'),10);assert.equal(h.run('window.ChapterPractice.questions(CHAPTER_ID).length'),10);
