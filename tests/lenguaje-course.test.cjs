...
const files=['courses.js','history-catalog.js','peru-catalog.js','lenguaje-catalog.js','history-progress.js','app.js'];
...
async function validateAppAndProgress(){
 ...
 for(let n=1;n<=6;n++){run(`setTopic(${n-1})`);assert.equal(h.redirects.at(-1),`lenguaje-capitulo-0${n}.html?v=20260918-language1`);}
 ...
 assert.match(home.run('renderProgressPanel()'),/Lenguaje · Capítulo 3/);home.run('openProgressChapter()');assert.equal(home.redirects.at(-1),'lenguaje-capitulo-03.html?v=20260918-language1');
 ...
 for(let n=1;n<=6;n++){
  ...
  assert.deepEqual(direct.redirects,[`index.html?chapter=${id}&v=20260918-language1`]);
  const entry=harness(files,{search:`?chapter=${id}`});await entry.signIn({uid:'new'});assert.equal(entry.run('state.view'),'mode');entry.run("selectStudyMode('progressive')");assert.equal(entry.redirects.at(-1),`${id}.html?v=20260918-language1`);
...
function validateTeacherTracking(){
 ...
 assert.equal(Object.keys(ctx.window.tracked).length,20);
