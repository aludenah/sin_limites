...
function illustrationMarkup(slide,cover=false){
 const visual=slide.illustration;if(!visual)return '';
...
function dialog(type){
 ...
 let content,title;
 if(type==='notes'){const s=SLIDES[P.currentSlide];title=s.title;content=`<p>${esc(s.notes)}</p>`;}
 else if(type==='image'){const s=SLIDES[P.currentSlide];const v=s.illustration;if(!v)return;title='Imagen del tema';content=`<img class="expanded-visual" src="${esc(v.src)}" alt="${esc(v.alt)}"><p class="image-caption">${esc(v.caption)} · Ilustración creada con IA.</p>`;}
 ...
 d.classList.toggle('image-dialog',type==='image');d.showModal();
}
...
ROOT.addEventListener('click',e=>{
 ...
 else if(a==='index'||a==='notes'||a==='image')dialog(a);
