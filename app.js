...
function trackedProgressReaders(){return [...HISTORY_CHAPTERS.map(c=>['historia-universal-capitulo-'+String(c.number).padStart(2,'0'),data=>applyHistoryProgress(c.number,data)]),['fisica-capitulo-01',applyPhysicsProgress],['fisica-capitulo-02',applyVectorsProgress]];}
...
function renderProgressPanel(){
  ...
  const activeChapterName=physics?(chapterNumber===2?'Vectores':'Análisis dimensional'):(HISTORY_CHAPTERS.find(c=>c.number===chapterNumber)?.title||'La historia como ciencia');
...
function specialHistoryContent(course){
  ...
  if(course.id===12&&state.activeTopicIndex>=1&&state.activeTopicIndex<=5){
    ...
    if(c)return `<section class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8"><p class="text-xs font-bold uppercase tracking-wider text-red-600">Capítulo ${c.number} de 24 · Contenido desarrollado</p><h2 class="mt-3 text-3xl font-black text-blue-950">${esc(c.title)}</h2><p class="mt-4 text-slate-600">${esc(c.intro)}</p><p class="mt-3 text-sm text-slate-500">${c.items} temas · ${c.items} casos guiados · ${c.items*2} preguntas de control · 5 actividades · evaluación de 10 preguntas</p><a href="historia-universal-capitulo-${String(c.number).padStart(2,'0')}.html" class="inline-flex mt-6 rounded-xl bg-blue-950 px-5 py-3 text-sm font-bold text-white">Estudiar el capítulo →</a></section>`;
  }
  return null;
}
