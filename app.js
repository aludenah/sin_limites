...
const PHYSICS_CHAPTERS=window.PHYSICS_CHAPTERS||[{number:1,topicIndex:0,title:'Análisis dimensional',progressId:'fisica-capitulo-01'},{number:2,topicIndex:1,title:'Vectores',progressId:'fisica-capitulo-02'}];
...
function activePhysicsNumber(){
 if(Number(navigationProgress.lastCourseId)!==16)return 1;
 const byTopic=PHYSICS_CHAPTERS.find(c=>c.topicIndex===Number(navigationProgress.lastTopicIndex));if(byTopic)return byTopic.number;
 const byNumber=PHYSICS_CHAPTERS.find(c=>c.number===Number(navigationProgress.lastChapterNumber)&&(!c.sourceLabel||navigationProgress.lastChapterName===c.title));
 return byNumber?.number||(navigationProgress.lastChapterName==='Vectores'?2:1);
}
...
function continueStudyEntry(){
  ...
  if(entry?.chapter){window.location.replace(entry.chapter+'.html?v=20260918-social1');return;}
  if(entry?.course){state.activeCourseId=entry.course;state.activeTopicIndex=entry.course===16?(PHYSICS_CHAPTERS.find(c=>c.number===activePhysicsNumber())?.topicIndex||0):entry.course===12?Math.max(0,activeHistoryNumber()-1):entry.course===11?Math.max(0,activePeruNumber()-1):entry.course===7?Math.max(0,activeLanguageNumber()-1):SOCIAL_COURSES[entry.course]?activeSocialNumber(entry.course)-1:0;state.activeTab='Teoría';state.view='course';}
...
function renderProgressPanel(){
  ...
  const activeProgress=social?socialChapterProgress(navigationProgress.lastCourseId,chapterNumber):physics?physicsChapterProgress(chapterNumber):peru?peruChapterProgress(chapterNumber):language?languageChapterProgress(chapterNumber):historyChapterProgress(chapterNumber);
  const activeCourseName=social?social.name:physics?'Física':peru?'Historia del Perú':language?'Lenguaje':'Historia Universal';
  const activeChapterName=social?social.chapters.find(c=>c.number===chapterNumber)?.title:physics?PHYSICS_CHAPTERS.find(c=>c.number===chapterNumber)?.title:((peru?PERU_CHAPTERS:language?LANGUAGE_CHAPTERS:HISTORY_CHAPTERS).find(c=>c.number===chapterNumber)?.title||'La ciencia histórica');
  const total=10;
...
function renderCatalog(){
  ...
  const cards=courses.map(c=>{
    const tracked=SOCIAL_COURSES[c.id]?SOCIAL_COURSES[c.id].chapters.map(h=>[h.number,socialChapterProgress(c.id,h.number)]):c.id===16?PHYSICS_CHAPTERS.map(h=>[h.number,physicsChapterProgress(h.number)]):c.id===12?HISTORY_CHAPTERS.map(h=>[h.number,historyChapterProgress(h.number)]):c.id===11?PERU_CHAPTERS.map(h=>[h.number,peruChapterProgress(h.number)]):c.id===7?LANGUAGE_CHAPTERS.map(h=>[h.number,languageChapterProgress(h.number)]):[];
    const detailed=tracked.map(([number,progress])=>`<div class="mt-4"><div class="flex items-center justify-between text-[11px] font-bold"><span class="text-slate-400">Cap. ${number}${c.id===16?(PHYSICS_CHAPTERS.find(h=>h.number===number)?.sourceLabel?' del PDF':'')+' · '+esc(PHYSICS_CHAPTERS.find(h=>h.number===number)?.title):''}</span><span class="text-red-600">${clamp(progress.percent,0,100)}%</span></div><div class="mt-1.5 h-1.5 rounded-full bg-slate-100 overflow-hidden"><div class="h-full bg-red-500 rounded-full" style="width:${clamp(progress.percent,0,100)}%"></div></div></div>`).join('');
