...
function specialHistoryContent(course){
  ...
  if(physics)return `<section class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8"><p class="text-xs font-bold uppercase tracking-wider text-red-600">${esc(physics.sourceLabel)} · Contenido desarrollado</p><h2 class="mt-3 text-3xl font-black text-blue-950">${esc(physics.title)}</h2><p class="mt-4 text-slate-600">${esc(physics.intro)}</p><p class="mt-3 text-sm text-slate-500">Teoría y ejemplos resueltos · 10 problemas con solución</p><a href="${physics.progressId}.html?v=20260919-fisica19" class="inline-flex mt-6 rounded-xl bg-blue-950 px-5 py-3 text-sm font-bold text-white">Estudiar ${esc(physics.title)} →</a></section>`;
