...
let historyProgress={},historyRecords={},peruProgress={};
...
function trackedProgressReaders(){
  ...
  return [...ids.map(id=>[id,data=>applyHistoryRecord(id,data)]),...PERU_CHAPTERS.map(c=>[c.progressId,data=>applyPeruProgress(c.number,data)]),['fisica-capitulo-01',applyPhysicsProgress],['fisica-capitulo-02',applyVectorsProgress]];
}
...
function entryDestination(){
  ...
  const chapters=['fisica-capitulo-01','fisica-capitulo-02',...HISTORY_CHAPTERS.map(c=>'historia-universal-capitulo-'+String(c.number).padStart(2,'0')),...PERU_CHAPTERS.map(c=>'historia-del-peru-capitulo-'+String(c.number).padStart(2,'0'))];
...
function continueStudyEntry(){
  ...
  if(entry?.chapter){window.location.replace(entry.chapter+'.html?v=20260917-peru7');return;}
...
function renderProgressPanel(){
  ...
  const chapterNumber=physics?activePhysicsNumber():peru?activePeruNumber():activeHistoryNumber();
  const activeProgress=physics?(chapterNumber===2?vectorsProgress:physicsProgress):peru?peruChapterProgress(chapterNumber):historyChapterProgress(chapterNumber);
  const activeCourseName=physics?'Física':peru?'Historia del Perú':'Historia Universal';
  const activeChapterName=physics?(chapterNumber===2?'Vectores':'Análisis dimensional'):((peru?PERU_CHAPTERS:HISTORY_CHAPTERS).find(c=>c.number===chapterNumber)?.title||'La ciencia histórica');
  const total=10;
...
function renderCatalog(){
  ...
  const cards=courses.map(c=>{
    const tracked=c.id===16?[[1,physicsProgress],[2,vectorsProgress]]:c.id===12?HISTORY_CHAPTERS.map(h=>[h.number,historyChapterProgress(h.number)]):c.id===11?PERU_CHAPTERS.map(h=>[h.number,peruChapterProgress(h.number)]):[];
...
function renderCourse(){
  ...
  const syllabusNote=course.id===12?'<p class="mt-3 text-sm text-slate-500">Historia Universal: 40 capítulos; contenido desarrollado del 1 al 6.</p>':course.id===11?'<p class="mt-3 text-sm text-slate-500">Historia del Perú: 35 capítulos; contenido desarrollado del 1 al 6.</p>':'';
...
function openProgressChapter(){if(!readyToChooseCourse())return;const course=Number(navigationProgress.lastCourseId),prefix=course===16?'fisica':course===11?'historia-del-peru':'historia-universal',number=course===16?activePhysicsNumber():course===11?activePeruNumber():activeHistoryNumber();window.location.assign(prefix+'-capitulo-'+String(number).padStart(2,'0')+'.html?v=20260917-peru7');}
function goHome(){state.view='catalog';window.scrollTo(0,0);render()}
function setTopic(i){if(!readyToChooseCourse())return;if(state.activeCourseId===11&&Number.isInteger(i)&&i>=0&&i<6){window.location.assign('historia-del-peru-capitulo-'+String(i+1).padStart(2,'0')+'.html?v=20260917-peru7');return;}if(state.activeCourseId===12&&Number.isInteger(i)&&i>=0&&i<=5){window.location.assign('historia-universal-capitulo-'+String(i+1).padStart(2,'0')+'.html?v=20260917-peru7');return}if(state.activeCourseId===16&&[0,1].includes(i)){window.location.assign('fisica-capitulo-0'+(i+1)+'.html?v=20260917-peru7');return}state.activeTopicIndex=i;state.activeTab='Teoría';saveProgress();render();window.scrollTo({top:0,behavior:'smooth'})}
...
auth.onAuthStateChanged(async user=>{
  ...
  if(user){
    ...
    navigationProgress={};
    historyProgress={};historyRecords={};peruProgress={};
