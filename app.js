...
function continueStudyEntry(){
  ...
  if(entry?.course){state.activeCourseId=entry.course;state.activeTopicIndex=entry.course===12?Math.max(0,activeHistoryNumber()-1):entry.course===11?Math.max(0,activePeruNumber()-1):entry.course===7?Math.max(0,activeLanguageNumber()-1):SOCIAL_COURSES[entry.course]?activeSocialNumber(entry.course)-1:0;state.activeTab='Teoría';state.view='course';}
...
function renderProgressPanel(){
  ...
  const activeProgress=social?socialChapterProgress(navigationProgress.lastCourseId,chapterNumber):physics?(chapterNumber===2?vectorsProgress:physicsProgress):peru?peruChapterProgress(chapterNumber):language?languageChapterProgress(chapterNumber):historyChapterProgress(chapterNumber);
  const activeCourseName=social?social.name:physics?'Física':peru?'Historia del Perú':language?'Lenguaje':'Historia Universal';
  const activeChapterName=social?social.chapters.find(c=>c.number===chapterNumber)?.title:physics?(chapterNumber===2?'Vectores':'Análisis dimensional'):((peru?PERU_CHAPTERS:language?LANGUAGE_CHAPTERS:HISTORY_CHAPTERS).find(c=>c.number===chapterNumber)?.title||'La ciencia histórica');
  const total=10;
...
function renderCatalog(){
  ...
  const cards=courses.map(c=>{
    const tracked=SOCIAL_COURSES[c.id]?SOCIAL_COURSES[c.id].chapters.map(h=>[h.number,socialChapterProgress(c.id,h.number)]):c.id===16?[[1,physicsProgress],[2,vectorsProgress]]:c.id===12?HISTORY_CHAPTERS.map(h=>[h.number,historyChapterProgress(h.number)]):c.id===11?PERU_CHAPTERS.map(h=>[h.number,peruChapterProgress(h.number)]):c.id===7?LANGUAGE_CHAPTERS.map(h=>[h.number,languageChapterProgress(h.number)]):[];
...
function renderCourse(){
  ...
  const syllabusNote=SOCIAL_COURSES[course.id]?`<p class="mt-3 text-sm text-slate-500">${esc(course.name)}: ${course.topics.length} capítulos; contenido desarrollado del 1 al 6.</p>`:course.id===12?'<p class="mt-3 text-sm text-slate-500">Historia Universal: 40 capítulos; contenido desarrollado del 1 al 6.</p>':course.id===11?'<p class="mt-3 text-sm text-slate-500">Historia del Perú: 35 capítulos; contenido desarrollado del 1 al 6.</p>':course.id===7?'<p class="mt-3 text-sm text-slate-500">Lenguaje: 34 capítulos; contenido desarrollado del 1 al 6.</p>':'';
...
auth.onAuthStateChanged(async user=>{
  ...
  if(user){
    ...
    navigationProgress={};
    historyProgress={};historyRecords={};peruProgress={};languageProgress={};socialProgress={};
