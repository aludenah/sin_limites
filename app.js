...
const PHYSICS_MODULES=PHYSICS_CHAPTERS;
...
function restoredTopicIndex(course,index,topicName,catalogVersion){
  // Catalog 10 splits the former first chapter; stable names take precedence.
  if(course.id===16){
    const name=physicsTopicName(topicName);
    const named=course.topics.findIndex(title=>physicsTopicName(title)===name);
    if(name&&named>=0)return named;
    if(['vectores','repaso vectores'].includes(name))return 1;
    if(name==='magnitudes y analisis vectorial')return 0;
    if(name==='electrodinamica y circuitos electricos')return 15;
    if(Number(catalogVersion)>=10)return clamp(index,0,course.topics.length-1);
    if(Number(catalogVersion)>=9){const previous=clamp(index,0,17);return previous===0?0:previous+1;}
    ...
    // Legacy 25-topic catalog already separated dimensional and vector analysis.
    // Thermal topics are absent from this volume and return to the opening chapter.
    return [0,1,2,2,3,4,5,8,8,7,8,8,9,10,10,0,0,13,13,14,14,15,16,11,18][splitIndex];
  }
...
function loadLocalProgress(){
  try{
    ...
    if(course){
      ...
      if(module)state.activeTopicIndex=module.topicIndex;
      navigationProgress={lastCourseId:course.id,lastCourseName:course.name,lastTopicIndex:state.activeTopicIndex,lastChapterNumber:state.activeTopicIndex+1,lastChapterName:module?.title||course.topics[state.activeTopicIndex],lastModuleId:state.activeModuleId,catalogVersion:CATALOG_VERSION};
      saveLocalProgress();
    }
  }catch(e){}
}
  ...
  try{
    ...
    if(snap.exists){
      ...
      if(COURSES.some(c=>c.id===courseId)){
        ...
        const module=PHYSICS_MODULES.find(m=>m.progressId===state.activeModuleId);
        if(module)state.activeTopicIndex=module.topicIndex;
        navigationProgress={...p,lastTopicIndex:state.activeTopicIndex,lastChapterNumber:state.activeTopicIndex+1,lastChapterName:module?.title||course.topics[state.activeTopicIndex],lastModuleId:state.activeModuleId,catalogVersion:CATALOG_VERSION};
        saveLocalProgress();
      }
    }
    cloudProgressReady=true;
...
function physicsTopicName(name){return String(name||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/^repaso(?: complementario)?:?\s*/,'').trim();}
function restoredPhysicsModuleId(record){
 const name=physicsTopicName(record.lastChapterName||record.activeTopicName);
 const named=PHYSICS_MODULES.find(m=>physicsTopicName(m.title)===name||(m.number===2&&name==='vectores')||(m.number===16&&name==='electrodinamica y circuitos electricos'));
 if(named)return named.progressId;
 // A named syllabus topic wins over IDs left behind by older merge writes.
 if(COURSES.find(c=>c.id===16)?.topics.some(title=>physicsTopicName(title)===name))return null;
...
function continueStudyEntry(){
  ...
  if(entry?.chapter){window.location.replace(entry.chapter+'.html?v='+(entry.chapter.startsWith('fisica-')?'20260919-fisica19':'20260918-social1'));return;}
  if(entry?.course){state.activeCourseId=entry.course;state.activeTopicIndex=entry.course===16?clamp(navigationProgress.lastTopicIndex,0,COURSES.find(c=>c.id===16).topics.length-1):entry.course===12?Math.max(0,activeHistoryNumber()-1):entry.course===11?Math.max(0,activePeruNumber()-1):entry.course===7?Math.max(0,activeLanguageNumber()-1):SOCIAL_COURSES[entry.course]?activeSocialNumber(entry.course)-1:0;state.activeTab='Teoría';state.view='course';}
...
function renderCourseCards(courses){
  return courses.map(c=>`<button onclick="openCourse(${c.id})" class="course-card bg-white rounded-3xl border border-slate-200 p-5 text-left group w-full flex flex-col"><div class="flex items-start justify-between gap-4 w-full"><div class="w-12 h-12 rounded-2xl bg-blue-50 text-blue-950 flex items-center justify-center group-hover:bg-blue-950 group-hover:text-white transition"><i data-lucide="${esc(c.icon)}" class="w-6 h-6"></i></div><span class="text-[11px] font-bold uppercase tracking-wider text-slate-400 bg-slate-50 px-2.5 py-1 rounded-full">${esc(c.category)}</span></div><h3 class="mt-5 mb-5 text-lg font-extrabold text-slate-900 leading-snug">${esc(c.name)}</h3><div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between w-full"><div class="flex items-center gap-2 text-sm font-semibold text-slate-500"><i data-lucide="layers-3" class="w-4 h-4"></i>${c.topics.length} capítulos</div><span class="text-blue-950 group-hover:text-red-600 transition"><i data-lucide="arrow-right" class="w-5 h-5"></i></span></div></button>`).join('');
}
...
function specialHistoryContent(course){
  ...
  if(course.id===7&&state.activeTopicIndex>=0&&state.activeTopicIndex<6){
    ...
    if(c)return `<section class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8"><p class="text-xs font-bold uppercase tracking-wider text-red-600">${esc(course.name)} · Capítulo ${c.number} de ${window.LANGUAGE_SYLLABUS.length}</p><h2 class="mt-3 text-3xl font-black text-blue-950">${esc(c.title)}</h2><a href="lenguaje-capitulo-${String(c.number).padStart(2,'0')}.html?v=20260918-social1" class="inline-flex mt-6 rounded-xl bg-blue-950 px-5 py-3 text-sm font-bold text-white">Estudiar el capítulo →</a></section>`;
  }
  if(course.id===11&&state.activeTopicIndex>=0&&state.activeTopicIndex<6){
    ...
    if(c)return `<section class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8"><p class="text-xs font-bold uppercase tracking-wider text-red-600">${esc(course.name)} · Capítulo ${c.number} de ${window.PERU_SYLLABUS.length}</p><h2 class="mt-3 text-3xl font-black text-blue-950">${esc(c.title)}</h2><a href="historia-del-peru-capitulo-${String(c.number).padStart(2,'0')}.html?v=20260918-social1" class="inline-flex mt-6 rounded-xl bg-blue-950 px-5 py-3 text-sm font-bold text-white">Estudiar el capítulo →</a></section>`;
  }
  if(course.id===12&&state.activeTopicIndex===0)return `<section class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8"><p class="text-xs font-bold uppercase tracking-wider text-red-600">${esc(course.name)} · Capítulo 1 de 40</p><h2 class="mt-3 text-3xl font-black text-blue-950">La ciencia histórica</h2><a href="historia-universal-capitulo-01.html?v=20260918-social1" class="inline-flex mt-6 rounded-xl bg-blue-950 px-5 py-3 text-sm font-bold text-white">Estudiar el capítulo →</a></section>`;
