...
function cloudPayload(){return {...P,...window.ChapterPractice.summary(CHAPTER_ID,P),courseId:16,courseName:'Física',chapterNumber:null,contentType:'review',moduleId:CHAPTER_ID,chapterName:CONTENT.title,schemaVersion:3,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};}
...
function render(){
  ...
  root.innerHTML=`${header()}<section class="hero"><div class="wrap"><div><p class="eyebrow">Física · Repaso complementario</p><h1>Vectores</h1><p>De una flecha a una ecuación. Representa, suma y descompón vectores; comprende sus productos y sus aplicaciones.</p></div><div class="vector-hero">${CONTENT.hero}</div></div></section><div class="wrap"><section class="card overview"><div><div class="progress-head"><span>Mi avance en el repaso</span><strong>${percent}%</strong></div><progress max="100" value="${percent}" aria-label="Avance del repaso">${percent}%</progress><small>${P.practice10.mastered.length}/10 problemas resueltos correctamente</small><p id="save-status" class="save-line ${cloudReady?'':'warning'}" role="status">${escapeHTML(saveMessage)}</p><button id="retry-save" class="button secondary small" data-action="sync" ${cloudReady?'hidden':''}>Reintentar sincronización</button></div></section><div class="layout"><aside class="card sidebar"><p class="eyebrow">Tu ruta de aprendizaje</p><nav class="route" aria-label="Temas del repaso">${side()}</nav><p class="hint">Completa los 10 problemas de la práctica para alcanzar el 100%. La teoría está disponible para consultar.</p></aside><main class="workspace" id="chapter-content"><nav class="card tabs" aria-label="Secciones del repaso">${[['theory','Teoría'],['practice','Práctica · 10 problemas'],['resources','Materiales']].map(([id,title])=>`<button data-action="tab" data-value="${id}" class="${P.activeTab===id?'active':''}" ${P.activeTab===id?'aria-current="page"':''} >${title}</button>`).join('')}</nav><article class="card article">${notice?`<div role="alert" class="feedback" id="notice">${escapeHTML(notice)}</div>`:''}${P.activeTab==='theory'?lessonView():P.activeTab==='practice'?practiceView():resourcesView()}</article></main></div></div><footer class="footer">SIN LÍMITES · Física · Vectores</footer>`;
...
async function signedIn(u){
  ...
  if(!u){root.innerHTML=`${header()}<main id="chapter-content" class="access card"><p class="eyebrow">Física · Repaso complementario</p><h1>Vectores</h1><p>Inicia sesión en la academia para estudiar y guardar tu avance.</p><a class="button" href="index.html?chapter=fisica-capitulo-02">Continuar con Google</a></main>`;return;}
 ...
 if(epoch!==authEpoch)return;
  ...
  const navigation={catalogVersion:9,lastCourseId:16,lastCourseName:'Física',lastTopicIndex:0,lastChapterNumber:1,lastModuleId:CHAPTER_ID,lastChapterName:'Repaso: '+CONTENT.title};
  try{localStorage.setItem('academia-sm-state',JSON.stringify({catalogVersion:9,activeCourseId:16,activeTopicIndex:0,activeModuleId:CHAPTER_ID,activeTopicName:'Magnitudes y análisis vectorial'}));}catch{}
