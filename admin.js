...
function renderRows(students){
  ...
  body.innerHTML=students.map(s=>`<tr class="hover:bg-slate-50"><td class="px-5 py-4"><div class="font-bold text-slate-800">${esc(s.displayName||'Sin nombre')}</div><div class="mt-0.5 text-xs text-slate-400">${esc(s.email||'')}</div></td><td class="px-5 py-4 min-w-48"><div class="flex items-center justify-between gap-3"><span class="font-extrabold ${s.percent>=100?'text-emerald-600':s.percent>0?'text-blue-950':'text-slate-400'}">${s.percent}%</span><span class="text-[11px] font-bold text-slate-400">${s.percent>=100?'Completado':s.percent>0?'En progreso':'Sin iniciar'}</span></div><div class="mt-2 h-2 rounded-full bg-slate-100 overflow-hidden"><div class="h-full rounded-full ${s.percent>=100?'bg-emerald-500':'bg-red-500'}" style="width:${s.percent}%"></div></div></td><td class="px-5 py-4 font-bold text-slate-700">${s.completedItems}/5</td><td class="px-5 py-4 font-bold text-slate-700">${s.attempts}</td><td class="px-5 py-4"><div class="font-semibold text-slate-700">${esc(s.lastCourse||'—')}</div><div class="text-xs text-slate-400 mt-0.5 max-w-56 truncate">${esc(s.lastChapter||'Sin actividad')}</div></td><td class="px-5 py-4 text-xs text-slate-500 whitespace-nowrap">${esc(formatDate(s.lastActivity))}</td></tr>`).join('');
}
...
async function fetchStudents(){
  ...
  const rows=await Promise.all(snap.docs.map(async userDoc=>{
    ...
    const [navSnap,progressSnap]=await Promise.all([
      ...
      ref.doc('historia-universal-capitulo-01').get()
    ]);
    ...
    return {
      ...
      attempts:sumAttempts(progress.attempts),
      lastCourse:nav.lastCourseName||'',
      lastChapter:nav.lastChapterName||'',
      lastActivity
    };
  }));
  allStudents=rows.filter(Boolean).sort((a,b)=>timestampMillis(b.lastActivity)-timestampMillis(a.lastActivity));
}
...
function exportCSV(){
  const header=['Nombre','Correo','Avance (%)','Ítems aprobados','Intentos','Último curso','Último capítulo','Última actividad'];
  const rows=allStudents.map(s=>[s.displayName,s.email,s.percent,s.completedItems,s.attempts,s.lastCourse,s.lastChapter,formatDate(s.lastActivity)]);
