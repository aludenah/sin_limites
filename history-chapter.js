...
function chapterLinks(){return `<nav class="chapter-links" aria-label="Cambiar de capítulo"><a href="historia-universal-capitulo-${String(CHAPTER_NUMBER-1).padStart(2,'0')}.html?v=20260917-reading3">← Capítulo ${CHAPTER_NUMBER-1}</a>${CHAPTER_NUMBER<6?`<a href="historia-universal-capitulo-${String(CHAPTER_NUMBER+1).padStart(2,'0')}.html?v=20260917-reading3">Capítulo ${CHAPTER_NUMBER+1} →</a>`:'<a href="index.html?course=12">Volver al temario →</a>'}</nav>`;}
...
root.addEventListener('change',event=>{
  ...
  if(el.id==='chapter-select'){const n=Number(el.value);if(HISTORY_CHAPTERS.some(c=>c.number===n))window.location.assign('historia-universal-capitulo-'+String(n).padStart(2,'0')+'.html?v=20260917-reading3');return;}
